...
[FlashAttention 算子实现](https://github.com/Wenyueh/MinivLLM/blob/main/src/myvllm/layers/attention.py#L112)
...
Gumbel-max 定理指出：如果 $G_i \sim \text{Gumbel}(0, 1)$ 是独立同分布的 Gumbel 随机变量，则从概率分布 $p_1, p_2, \ldots, p_V$ 中采样，等价于 $\arg\max_i (\log p_i + G_i)$。标准Gumbel分布可通过 G = -log(-log(U)) 生成，其中 U ~ Uniform(0,1)。
$\arg\max_i (log(p_i) + G_i) = \arg\max_i (log(p_i) - log(-log(U_i))) = \arg\max_i log(p_i / (-log(U_i))) = \arg\max_i (p_i / X_i)$，其中 $X_i = -log(U_i) \sim Exponential(1)$。

为什么不直接用 torch.multinomial 采样？
...
在常规 PyTorch / CUDA 推理中，每一个算子往往都会经历一次 CPU 向 GPU 提交内核（kernel launch） 的过程。CUDA Graph 是 NVIDIA 提供的一种机制：把一整段已经确定好的 GPU 操作序列 预先录制（capture） 成一张 有向无环图（DAG），之后在运行时只需 replay() 重放 这张图，而无需在每一步都重复走完整的 launch 路径。CUDA Graph 由两部分组成：

- 图定义：一个包含多个 节点 和 边 的有向无环图。每个节点代表一个 GPU 操作（如 kernel launch、内存拷贝），边代表依赖关系。
- 图实例：从图定义创建的可执行对象，可以在 GPU 上直接提交执行。
...
- 计算图结构固定：每步每个序列只处理 1 个 token，Transformer 各层的矩阵形状完全可预测。
- 小规模计算高频执行：每步计算量不大，但 kernel launch 的次数多（LayerNorm、QKV 投影、注意力、FFN 等），launch 开销占比显著。
- CPU 成为瓶颈：GPU 很快就能算完一步的矩阵运算，但 CPU 忙于提交下一步的 kernel，导致 GPU 空等。
...
硬编码上限 512 是工程经验值：batch 超过 512 时，计算本身已经能充分利用 GPU，launch overhead 占比下降，Graph 收益变小。同时，Graph 数量越多，capture 耗时和显存开销越大，512 是一个合理的上限折中。

为什么不是每个 batch size 都录制一张图：

- 内存成本：每张图都会固定一份 GPU 内存，图太多会浪费显存。
- capture 耗时：每张图需要 warmup + capture，数量越多初始化越慢。
...
CUDA Graph 的内存池（graph_pool）在首次使用时会根据需求分配内存。从 最大的 batch 开始录制，能确保内存池一开始就分配了足够大的空间。PyTorch 允许把第一张图的 graph.pool() 传给后续 torch.cuda.graph(..., pool=)，使多张图共享同一套内存池。后续较小的 batch 只会使用这片空间的子集，不需要额外扩展，减少了碎片和重新分配。
...
如果把这些不稳定行为录进图里，replay 时会得到错误结果或崩溃。因此必须先 warmup 再 capture。

CUDA Graph 内部记录的是 tensor 的内存地址，不是 tensor 对象。因此 replay 前需要把真实数据 拷入 capture 时使用的同一块内存中。graph_vars 保存了这些 tensor 的引用。尽管 Attention 与 KV 写入 kernel 代码中读取了全局上下文，但在 replay 中它们并不直接读 Python 里的 context 对象，而是依赖 set_context 在 capture 时已绑定到的 graph_vars。简单来说，capture 时，set_context 传入的 tensor，其地址被绑定到了 graph_vars，replay 时，涉及到读取 context 的 kernel 就会自动从 graph_vars 中读取。
...
slot_mapping.fill_(-1) 是为了确保 replay 时，slot_mapping 的值不会被上次计算残留的值影响。Attention 里的 Triton store_kvcache_kernel 对 slot == -1 会直接 return，与这一约定一致。同理，context_lens 使用 zero_()。

调用 graph.replay() 时：

- CPU 侧：只需要一次 cudaGraphLaunch 调用，将整个 DAG 提交给 GPU。
- GPU 侧：按照 capture 时录制的顺序与依赖关系，依次执行所有 kernel。
- 数据传递：Graph 内部各节点使用的是 capture 时绑定的内存地址，所以预先往 graph_vars 拷入数据后，Graph 内部自然能读到最新输入。
- 输出获取：outputs[:bs] 在 replay 完成后自动包含最新的模型输出，因为它也是 capture 时绑定的缓冲区。

注意 compute_logits 是在 graph.replay() 之后 调用的，并没有被录入 Graph。原因是 compute_logits 通常涉及 lm_head 权重矩阵乘法，其输出的 logits shape 依赖于词表大小，且可能需要配合采样等后续操作。将其放在 Graph 外部更灵活。

enforce_eager = True，启动时不录制任何图，适合场景：
...
decoder 阶段，cuda graph 加速比可达到 1.5~2倍，batch 越小，Graph 带来的加速比越大，因为小 batch 时 launch overhead 占比更高。Graph capture 需要在启动时额外执行 warmup + capture，典型耗时：7B 模型、20 个档位，约 30-60 秒。

## Triton Kernel

Triton 是由 OpenAI 开发的一门 GPU 编程语言和编译器，目标是让开发者用 接近 Python 的语法 编写高性能 GPU 内核（kernel），而无需掌握 CUDA C++ 的底层细节。Triton 能在 保持接近 CUDA 的性能 的同时，大幅降低开发门槛，这使得它成为深度学习系统中编写自定义 kernel 的首选工具。
...
- 标记 JIT 编译：函数体不会立即执行，而是在首次被调用时，Triton 编译器将其编译为 GPU 机器码（PTX → SASS）。
- 类型推导：根据调用时传入的参数类型自动推导内部变量类型。
- 特化（Specialization）：tl.constexpr 参数会在编译期固化，编译器可以据此做常量折叠、循环展开等优化。
- 缓存：编译结果会被缓存，相同参数类型的后续调用不需要重新编译。
...
store_kvcache_kernel[(N,)](...) 中 [(N,)] 定义了 Triton kernel 的 grid 大小，等价于 CUDA 的 gridDim。这里只用了 1 维 grid，N 个 program 分别处理 N 个 token。在 CUDA 中，等价写法大致为：store_kvcache_kernel<<<N, 1>>>(...)。但 Triton 中每个 "program" 内部可以处理多个元素（通过 tl.arange），所以不需要像 CUDA 那样显式指定 blockDim。
...
Triton 编译器自动进行内存访问优化：

- 访问合并（Coalescing）：当多个线程访问连续地址时，合并为一次内存事务。
- 向量化（Vectorization）：将多个标量 load/store 合并为 128-bit 宽的向量操作。
- 寄存器分配：自动决定哪些中间值放在寄存器中。

Triton kernel 与 CUDA Graph 完全兼容。在 capture_cudagraph 期间：

- kernel 首次执行时完成 JIT 编译（warmup 阶段）。
- capture 阶段的 torch.cuda.graph(...) 会录制 Triton kernel 的调用。
- replay 时，Triton kernel 像普通 CUDA kernel 一样被重放。

业界常用 Triton 实现的 LLM 相关 kernel 包括：
...
Prefill 吞吐量 衡量引擎处理 prompt 的速度，与 计算能力（FLOPS） 相关（prefill 是计算密集型），长 prompt 时更能体现差异。

Decode 吞吐量 衡量引擎生成新 token 的速度，与 内存带宽 和 调度效率 相关（decode 是内存密集型）。batch 越大，GPU 利用率越高，decode 吞吐量越高。

$ \text{Total Throughput} = \frac{\text{prefill tokens} + \text{decode tokens}}{\text{total time}} $

### bench.py

测试 decode 侧按 max_tokens 约定总生成量 下的平均吞吐。
...
- CUDA 上下文初始化：首次使用 GPU 时需要初始化驱动、分配设备内存。
- torch.compile JIT 编译：@torch.compile 修饰的函数在首次调用时编译，后续调用使用缓存。
- CUDA Graph capture：已在 ModelRunner 初始化时完成，但首次 replay 可能有额外开销。
- cuBLAS handle 初始化：首次矩阵乘会创建和配置 cuBLAS 句柄。
- 内存分配器预热：PyTorch 的 CUDA 内存分配器在首次分配后会缓存内存块。
...
- Chunked Prefill 缺失（存疑）：vLLM 可以将长 prompt 分块处理，与 decode 交错执行，提高 GPU 利用率。
- 调度策略简化：nano-vllm 的调度器不如 vLLM 精细（如缺少 priority 调度、preemption 机制）。
- Paged Attention V2 缺失：vLLM 使用了更优化的分页注意力实现。
- 量化支持缺失：AWQ/GPTQ 等量化可以减少内存带宽需求，提升 decode 速度。
- 更精细的内存管理：vLLM 的 BlockManager 有更细粒度的 swap/recompute 策略。
...
建议设 max_num_seqs 为能容纳在 GPU 显存中的最大值。

gpu_memory_utilization（0.85-0.95）留余量避免 OOM，但不宜过低。enforce_eager（False）除非调试，否则应启用 CUDA Graph。enable_prefix_caching（True）共享前缀场景下显著加速。
...
[张量并行与序列并行](https://littlenyima.github.io/posts/58-ultrascale-playbook-tensor-parallelism/index.html)

nano-vllm 实现了简单的张量并行，但存在一些问题。

张量并行也引入了显著的通信需求，这严重依赖于设备的通信带宽。这种通信无法和计算过程并行，因此会导致前向传播所需的时间增加。虽然增加张量并行度会导致每 GPU 吞吐量降低，但它能够处理更大的批次大小，说明了分布式训练中计算效率与内存可用性之间的权衡。当扩展到 8 个 GPU 以上时，张量并行的通信开销变得尤为明显。虽然但节点内的张量并行可以利用快速的 NVLink 进行互联，但跨节点则需要较慢的网络连接。因此可以观察到从 TP=8 到 TP=16 有显著的下降，从 TP=16 到 TP=32 的下降更为剧烈。

此外，不管是 o_proj 还是 v_proj，做 all_reduce 后，每个 GPU 会得到完整的序列，各自通过 layerNorm，加残差等操作，这导致了无意义的计算量增加，显存占用也增加了。因此一个改进方法是引入序列并行，对张量并行无法处理的模型部分（例如 Dropout 以及 LayerNorm）的激活值和计算进行拆分。之所以需要引入序列并行，是因为这些操作需要完整的隐藏层维度数据来进行计算。

对于每个 GPU 中 o_proj 或 v_proj 的输出，我们不做 all-reduce，而是 reduce-scatter，这样每个 GPU 只会得到部分序列。然后每个 GPU 对自己的输出进行 layerNorm，加残差等操作，最后再进行一次 all-gather，得到完整的输出，传递给下一个需要 tp 的模块。这样，我们就可以避免无意义的计算量增加，显存占用也减少了。
