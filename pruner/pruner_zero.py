...
def prepare_calibration_input(model, layers, dataloader, device, seq_len=2048):
...
def prune_pruner_zero(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    # engine = GPTree.load_tree("LlmPruner/best_tree.json")

    with torch.no_grad():
        inps, outs, cache = prepare_calibration_input(model, layers, dataloader, device, seq_len=2048)
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            X = wrapped_layers[name].scaler_row.reshape((1, -1)).to(device=W.device, dtype=torch.float32)
            G = gradients[indexed_name].to(device=W.device, dtype=torch.float32)
            print(f"pruning layer {i} name {name}")

            W_metric = torch.abs(W * W) * mms(torch.abs(G))
            ...
            if prune_n != 0:
                ...
                if args.use_variant:
                    ...
                    indices = sort_res[1][:, :int(W_metric.shape[1] * p_ratio)]
...
class PrunerZeroPruner:
    ...
    def prune(self, args):
        ...
        prune_pruner_zero(args, self.model, self.tokenizer, device=self.model.device, prune_n=args.N, prune_m=args.M)
