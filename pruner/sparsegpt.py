...
def prune_sparsegpt(args, model, tokenizer, dev, prune_n=0, prune_m=0):
    ...
    # if prune_n != 0:
    #     flag = (prune_n / prune_m < 0.3)
    # else:
    #     flag = args.final_s > 0.75
    ...
    for i in tqdm(range(len(layers))):
        ...
        for j in range(args.num_examples):
            with torch.no_grad():
                layer(inps[j].unsqueeze(0), **cache)
        for h in handles:
            h.remove()

        # print(f"sgpt pruning layer {i} ")
        for j, name in enumerate(subset):
            start_time = time.time()
            p_ratio = args.all_layer_ratio[i]
            # p_ratio = args.final_s
            gpts[name].fasterprune(p_ratio,
                                   prune_n=prune_n, prune_m=prune_m, percdamp=percdamp, blocksize=128)
