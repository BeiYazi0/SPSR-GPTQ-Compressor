...
class QuantLlamaAttention(nn.Module):
    def __init__(self, hidden_size, num_heads, qkv_proj, o_proj, config, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.q_out = self.num_heads * self.head_dim
        self.kv_out = self.num_key_value_heads * self.head_dim
    ...
    def forward(
        ...
        **kwargs,
        ...
        qkv = self.qkv_proj(hidden_states)       # [bsz, q_len, q_out + kv_out + kv_out]

        q_end = self.q_out
        k_end = q_end + self.kv_out
...
def make_quant_attn(model):
    ...
    for name, m in model.named_modules():
        ...
        attn = QuantLlamaAttention(hidden_size, num_heads, qkv_layer, m.o_proj, model.config, layer_idx=int(name.split('.')[2]))
