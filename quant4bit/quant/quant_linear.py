...
class QuantLinear(nn.Module):
    ...
    def pack(self, linear, scales, zeros, g_idx=None):
        ...
        intweight = intweight.to(torch.int32)  # 转换为 int32 类型，shape(infeatures, outfeatures)
        qweight = torch.zeros(
            (intweight.shape[0] // 32 * self.bits, intweight.shape[1]), # 每 32 // bits 个输入通道打包成一个 int32
            dtype=torch.int32,
