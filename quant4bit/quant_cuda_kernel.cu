    ...
    int vec_height)
{
    ...
    // if (vec.scalar_type() == at::ScalarType::Half)
    // {
    //     VecQuant4MatMulKernel_G<half><<<blocks, threads>>>(
    //         (half2 *)vec.data_ptr<half>(),
    //         mat.data_ptr<int>(),
    //         mul.data_ptr<half>(),
    ...
    //         batch, vec_height, height, width, zero_width);
    // }
    // else if (vec.scalar_type() == at::ScalarType::Float)
    // {
    //     VecQuant4MatMulKernel_G<float><<<blocks, threads>>>(
    //         (half2 *)vec.data_ptr<half>(), // 注意：vec 仍然是 half2 类型
    //         mat.data_ptr<int>(),
    //         mul.data_ptr<float>(),
    //         scales.data_ptr<float>(),
    //         zeros.data_ptr<int>(),
    ...
    // else
    // {
    //     TORCH_CHECK(false, "vecquant4matmul_g_cuda not implemented for ", vec.scalar_type());
    // }

    // AT_DISPATCH_FLOATING_TYPES(vec.scalar_type(), "vecquant4matmul_g_cuda", [&]
    //                            { VecQuant4MatMulKernel_G<scalar_t><<<blocks, threads>>>(
    //                                  (half2 *)vec.data_ptr<half>(), // vec 始终是 half2
    //                                  mat.data_ptr<int>(),
    //                                  mul.data_ptr<scalar_t>(),
    //                                  scales.data_ptr<scalar_t>(),
    //                                  zeros.data_ptr<int>(),
